...
export default function Home() {
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* Experience Section */}
      <section id="experience" className="py-20 hero-gradient text-white">
        <div className="container mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                  <div>
                    ...
                    {exp.companyUrl ? (
                      <a href={exp.companyUrl} target="_blank" rel="noopener noreferrer" className="text-xl font-semibold text-blue-300 hover:text-blue-100 underline">{exp.company}</a>
                    ) : (
                      <p className="text-xl font-semibold">{exp.company}</p>
                    )}
                    {exp.department && <p className="text-pink-200">{exp.department}</p>}
                  </div>
                  <span className="text-pink-200 font-semibold">{exp.duration}</span>
                </div>
                <p className="text-pink-100 mb-4">{exp.description}</p>
                ...
                {exp.projects && exp.projects.map((project, pIndex) => (
                  <div key={pIndex} className="bg-black/20 p-4 rounded-lg mb-4">
                    ...
                    {'url' in project ? (
                      ...
                      <h5 className="text-white font-semibold mb-3">{project.title}</h5>
                    )}
                    <ul className="space-y-1">
                      {project.achievements.map((achievement, aIndex) => (
                        <li key={aIndex} className="flex items-start text-pink-100">
                ...
                {exp.genai && (
                  <div className="bg-purple-900/30 p-4 rounded-lg mb-4">
                    <h4 className="text-purple-300 font-bold mb-3">{exp.genai.title}</h4>
                    <ul className="space-y-1 mb-3">
                      {exp.genai.achievements.map((achievement, aIndex) => (
                        <li key={aIndex} className="flex items-start text-pink-100">
                    ...
                    <p className="text-sm text-pink-200">
                      <strong>Skills:</strong> {exp.genai.technologies.join(', ')}
                    </p>
                  </div>
                )}
                ...
                {exp.skills && (
                  <div className="bg-black/20 p-4 rounded-lg mb-4">
                    <h4 className="text-green-400 font-bold mb-2">Key Skills:</h4>
                    <div className="grid md:grid-cols-2 gap-2">
                      {Object.entries(exp.skills).map(([category, skills]) => (
                        <div key={category} className="text-pink-100 text-sm">
                ...
                {exp.researchUrl && (
                  <div className="bg-purple-900/30 p-4 rounded-lg">
                    <h4 className="text-purple-300 font-bold mb-2">Research Publications:</h4>
                    <a href={exp.researchUrl} target="_blank" rel="noopener noreferrer" className="text-blue-300 hover:text-blue-100 underline">
                      View all publications on ResearchGate
                    </a>
                  </div>
                )}
                
                {/* Thesis Link for other positions */}
                {exp.thesis && exp.thesisUrl && (
                  <div className="bg-green-900/30 p-4 rounded-lg">
                    <h4 className="text-green-300 font-bold mb-2">Master Thesis:</h4>
                    <a href={exp.thesisUrl} target="_blank" rel="noopener noreferrer" className="text-blue-300 hover:text-blue-100 underline">
                      {exp.thesis}
                    </a>
                  </div>
                )}
